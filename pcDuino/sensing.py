...
Additionally, the external module (included in the source repository) pypcduino is required.
...
# @exception NoPinFilesException This exception will be thrown when the files to read pins are not present, most likely indicating that the code is not being run on a pcDuino.
def setupSensors():
...
class TemperatureSensor(object):
	...
	# @exception SensorPinException Thrown when the pin specified is not in the valid range
	...
	# @return Returns the AD reading as a calculated voltage
	...
	# @exception NoHistoryException Thrown when there are no readings in the history.
...
class LightSensor(object):
	...
	# @exception PinReadOutOfRangeException Thrown when the value read from a pin is not in the specified range.
	def update(self):
